...
const mockProduct = (product?: any): Product => {
  return {
    ...product,
...
describe("catalogService", () => {
  ...
  // set prerequsite need to run the test
  beforeEach(() => {
    repository = new MockCatalogRepository();
  });

  // once test done clear thing
  afterEach(() => {
  ...
  describe("deleteProducts", () => {
    ...
    test("should throw error with product doesn't deleted", async () => {
      const service = new CatalogService(repository);
      const product = ProductFactory.build();
      jest
        .spyOn(repository, "delete")
        .mockImplementationOnce(() =>
          Promise.reject(new Error("product doesn't deleted"))
        );

      await expect(service.deleteProduct(product.id!)).rejects.toThrow(
        "product doesn't deleted"
      );
    });
  });
});
