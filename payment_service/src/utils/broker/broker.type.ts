...
export type MessageBrokerType = {
  ...
  publish: (data: PublishType) => Promise<boolean>; // producer send message to topic

  // consumer
  connectConsumer: <T>() => Promise<T>;
  disconnectConsumer: () => Promise<void>;
  subscribe: (messageHandler: MessageHandler, topic: TOPIC_TYPE) => Promise<void>; // consumer listen message from topic
};
